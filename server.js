...
async function gql(query, variables = {}) {
  ...
  const response = await fetch(
    `https://${shopHost()}/admin/api/${API_VERSION}/graphql.json`,
    {
      method: "POST",
      headers: {
    ...
    ${
      last
        ? \`<div class="product">\${htmlEscape(last.productTitle)}</div>
           <div class="meta">
             SKU: \${htmlEscape(last.sku || "n/a")}<br>
             \${last.before} to \${last.after} | \${htmlEscape(last.timestamp || "")}
           </div>\`
        : \`<div class="meta">Scanner ready.</div>\`
    }
...
function focusBarcode() {
  if (input && document.activeElement !== pin) {
...
function isControl(el) {
  ...
  return tag === "input" || tag === "button" || tag === "a";
}

function protectFocus(e) {
...
document.addEventListener("mousedown", protectFocus, true);
document.addEventListener("touchstart", protectFocus, true);
document.addEventListener("pointerdown", protectFocus, true);
...
app.post("/scan", async (req, res) => {
  ...
  try {
    ...
    if (!barcode) {
      throw new Error("No barcode entered.");
    }
